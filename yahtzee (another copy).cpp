...
int getPlayerCount(){
    ...
    while(player_count == 0){
        std::cout << "How many players play (max = 9): ";
        std::string input_string = "";
        std::cin >> input_string;

        if(std::isdigit(input_string[0])){
            input_string = input_string[0];
            player_count = std::stoi(input_string);
        }
        else{
            std::cout << "Error! Give number between 1-9!\n";
        }
    }
...
void PlayerTurn(std::vector<Player*> p, Dices dices, int i){
    ...
    while(chosen <= -1){        
        ...
        if(std::isdigit(input_str[0])){
            ...
            //tarkastetaan onko input_str yli kymmnen(kaksinumeroinen)
            if(std::isdigit(input_str[1])){
            ...
            std::cout << "Error. Give number between 1-16!\n";
        }  

        if(chosen >= 1 && chosen <= 16 && chosen != 7){
            --chosen; //-1 muutetaan vastaamaan tulostaulun switchiä

            if(p[i]->score_array[chosen] == 0){
               // std::cout << "p[i]->score_array[chosen] = " << p[i]->score_array[chosen] << "\n";
            }
            else{
...
void showWinner(std::vector<Player*> p){
    ...
    //verrataan parasta lopputulosta kaikkiin kaksois- tai kolmoisvoiton, jne. varalta
    int winner_count = 0;
...
void printMenu(){
    ...
    std::cout << "| 3. Epstein didn't kill himself                             |\n";
    std::cout << "|                                                            |\n";
    std::cout << "--------------------------------------------------------------\n";
}
